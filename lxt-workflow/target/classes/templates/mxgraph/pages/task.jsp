				...
				<table class="workflow-config-table" width="100%">
					<tr height="50">
						<td width="35%" align="right">
							<label for="tc_label"><span class="required">*</span>任务名称：</label>
						</td>
						<td width="65%">
					...
					<tr>
						<td align="right">
							<label for="tc_form"><span class="required">*</span>业务表单：</label>
						</td>
						<td>
							<input id="tc_formText" name="formText" data-options="required:true,tipPosition:'bottom',validType:'username[4,32]'" class="easyui-validatebox searchbox" type="text" maxlength="100"/>
							<input type="hidden" id="tc_formId" name="formId"/>
						</td>
					</tr>
					<tr>
						<td align="right">
							<label for="tc_remindRule"><span class="required">*</span>催办规则：</label>
						</td>
						<td>
							<input id="tc_remindRule" name="remindRuleId" class="searchbox" type="text" maxlength="100"/>
						</td>
					</tr>
					<tr>
						<td align="right">
							<label for="tc_handlerType"><span class="required">*</span>参与类型：</label>
						</td>
						<td>
							<select id="tc_handlerType" name="handlerType" style="width:152px;">
					...
					<tr>
						<td align="right">
							<label for="tc_handler">处理人：</label>
						</td>
						<td>
							<input id="tc_handlerText" name="handlerText" readonly="readonly" data-options="validType:'length[0,256]'" class="easyui-validatebox searchbox" type="text" maxlength="100"/>
							<input id="tc_handlerId" name="handlerId" data-options="validType:'length[0,256]'" class="easyui-validatebox searchbox" type="hidden" maxlength="100"/>
						</td>
					</tr>
					<tr>
	...
	<script type="text/javascript">
		...
		$('#tc_handlerType').combobox({
			...
			onSelect:function(record){
				...
				if(value==0){
					Selector.showUserSelector($('#tc_handlerId'),$('#tc_handlerText'),true);
				}else if(value==1){
					Selector.showUserSelector($('#tc_handlerId'),$('#tc_handlerText'),true);
				}else if(value==2){
					Selector.showOrganizationSelector($('#tc_handlerId'),$('#tc_handlerText'),true);
				}else if(value==3){
					Selector.showDepartmentSelector($('#tc_handlerId'),$('#tc_handlerText'),true);
				}
			}
		});
		...
		function taskPropertyUpdate(cell){
			...
			cell.setAttribute('formId',$('#tc_formId').val());
			cell.setAttribute('formText',$('#tc_formText').val());
			cell.setAttribute('remindRuleId',$('#tc_remindRule').val());
			cell.setAttribute('remindRuleText',$('#tc_remindRuleText').val());
			cell.setAttribute('handlerType',$('#tc_handlerType').combo('getValue'));
			cell.setAttribute('handlerId',$('#tc_handlerId').val());
